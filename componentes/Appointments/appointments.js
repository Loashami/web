    ...
    function initComponent() {
      ...
      const openEditModal = (appointment) => {
        ...
        fields.client.value = appointment.client;
        fields.pet.value = appointment.pet;
        fields.service.value = appointment.service;
        modal.classList.remove("hidden");
      };

      const cancelAppointment = (id) => {
        const index = appointments.findIndex(a => a.id === id);
        if (index !== -1 && confirm("Are you sure you want to cancel this appointment?")) {
      ...
      const renderAppointments = () => {
        ...
        const filtered = appointments.filter(a => {
          ...
          const matchSearch = !query || a.client.toLowerCase().includes(query) || a.pet.toLowerCase().includes(query) || a.service.toLowerCase().includes(query);
        ...
        filtered.forEach(a => {
          ...
          tr.innerHTML = `
            ...
            <td class="p-4">
              <span class="px-2 py-1 rounded text-white text-xs ${getStatusClass(a.status)}">
                ${a.status.charAt(0).toUpperCase() + a.status.slice(1)}
              </span>
            </td>
            <td class="p-4 flex gap-2">
              <button class="px-2 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-600 transition" onclick='openEditModal(${JSON.stringify(a)})'>Edit</button>
              <button class="px-2 py-1 text-sm bg-red-500 text-white rounded hover:bg-red-600 transition" onclick='cancelAppointment("${a.id}")'>Delete</button>
            </td>
          `;
