    ...
    function renderUsers() {
      ...
      for (const user of filtered) {
        ...
        row.innerHTML = `
          ...
          <td class="p-3">${user.specialty || user.supervisor || (user.pets?.join(', ') || 'Admin Access')}</td>
          <td class="p-3">
    ...
    function setTab(tab) {
      activeTab = tab;
      renderUsers();
      document.querySelectorAll('.tab-btn').forEach(btn => btn.classList.remove('bg-blue-100', 'text-blue-600'));
